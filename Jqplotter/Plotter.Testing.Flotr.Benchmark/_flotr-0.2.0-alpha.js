...
// ----------------------------------------------------
// fixed IE9 support, added date time support in spreadsheet view 
// and updated to prototype 1.7.
// Updated constructCanvas and loadDataGrid
// ----------------------------------------------------
        ...
        **/
        for (i = 0; i < s.length; ++i) {
            s[i].data.each(function (v) {
                var x = v[0],
				    y = v[1];
                // Akram El Assas: added date time support
                if (r = dg.find(function (row) {
                    if (typeof (x) === 'object') {
                        return row[0].getTime() == x.getTime();
                    }
                    else {
                        return row[0] == x;
                    }
