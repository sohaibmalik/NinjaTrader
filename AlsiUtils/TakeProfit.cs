...
namespace AlsiUtils
{
	...
	public class TakeProfit
	{
		...
		{
			Debug.WriteLine("Starting MAMA :");
			Stopwatch sw = new Stopwatch();
			sw.Start();
			mama = Factory_Indicator.createAdaptiveMA_MAMA(0.5, 0.05, VarList);
			...
			Debug.WriteLine(" Start index  :" + ftl_Start_Index);
			...
			for (int x = 0; x < Ftl_Count-ftl_Start_Index; x++) FTL[ftl_Start_Index+x].Mama = mama[x];
				...
				sw.Stop();
			Debug.WriteLine("time to set mama : " + ((double)sw.ElapsedMilliseconds / (double)1000));
